...
public class Recognizer {
    ...
    List<Recognition> recognizeImage(Bitmap bitmap, Matrix matrix) {
        synchronized (this) {
            List<RectF> faces = blazeFace.detect(bitmap); //detecting face
            ...
            for (RectF rectF : faces) {
                ...
                FloatBuffer buffer = faceNet.getEmbeddings(bitmap, rect); //menghasilkan 512-d embedding
                LibSVM.Prediction prediction = svm.predict(buffer); //svm prediction with 512-d embedding
    ...
    void updateData(int label, ContentResolver contentResolver, ArrayList<Uri> uris) throws Exception {
        synchronized (this) {
            ...
            for (Uri uri : uris) {
                ...
                faceNet.getEmbeddings(bitmap, rect).get(emb_array);//get embedding
                list.add(emb_array);
            }

            svm.train(label, list); //train embedding
        }
    }
